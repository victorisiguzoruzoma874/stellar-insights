...
export function IssuedAssetsTable({ assets }: IssuedAssetsTableProps) {
                        ...
                        {assets.length === 0 ? (
                            ...
                            paginatedAssets.map((asset) => (
                                <tr key={asset.asset_code} className="hover:bg-slate-800/30 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className={`flex items-center justify-end gap-1.5 ${asset.failure_rate > 5 ? 'text-rose-400 font-medium' : 'text-slate-400'
                                            }`}>
                                            {asset.failure_rate > 5 && <AlertTriangle className="w-3.5 h-3.5" />}
